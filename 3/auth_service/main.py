...
async def create_master_user():
    async with database_pool.acquire() as conn:
        master_user = await conn.fetchrow(
...
async def create_user(user: UserCreate):
    ...
    async with database_pool.acquire() as conn:
        try:
            ...
            logger.info(f"Created new user: {user.username} with ID: {user_record['user_id']}")
            return UserPublic(
...
async def read_users(current_user: UserInDB = Depends(require_admin)):
    async with database_pool.acquire() as conn:
        users = await conn.fetch("SELECT user_id, username, full_name, role FROM users")
        return [
            UserPublic(
                user_id=user["user_id"],
                username=user["username"],
